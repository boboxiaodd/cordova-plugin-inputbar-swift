...
open class FDSoundActivatedRecorder: NSObject, AVAudioRecorderDelegate {
    ...
    //BOBO_MARK
    public var stopTriggerIntervals = 1000
    ...
    @objc dynamic open var microphoneLevel: Float = 1.0
    ...
    open func stopAndSaveRecording() {
        ...
        // Prepare output
        let trimmedAudioFileBaseName = "\(UUID().uuidString).caf"
    ...
    internal func interval(currentLevel: Float) {
        ...
        case .recording:
            ...
            if let triggerLevel = triggerLevel, currentLevel <= triggerLevel {
                ...
                if triggerCount == 1 {
                    self.delegate?.soundNotDelectSound(true)
                }
                if triggerCount >= stopTriggerIntervals {
                    stopAndSaveRecording()
                }
            } else {
                if triggerCount > 0 {
                    self.delegate?.soundNotDelectSound(false)
                }
                triggerCount = 0
